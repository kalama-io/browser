...
echo "--------------------------------------------------------------------------"
while true; do
    read -p "You are uninstall Kalama application. Are you sure [Y/N]? " answer
    case $answer in 
    Y | y | yes | YES) 
        echo "Begin uninstall application"
        break;;
    N | n | no | NO)
        echo "cancel uninstall Kalama application user"
        exit 0;;
...
browser_process_id=`ps -ef | grep "Kalama" | grep -v "grep" | awk '{print $2}'`
if [[ -n ${browser_process_id} ]]; then
    echo Kalama process is runing, please close the application.
    echo Kalama pid is $browser_process_id.
    sleep 5
    exit 0
fi
...
package_exist=`pkgutil --pkgs-plist | grep "com.Kalama.browser.package"`
if [[ -n ${package_exist} ]]; then
    sudo /usr/sbin/pkgutil --forget com.Kalama.browser.package
fi

echo "End uninstall Kalama application"
echo "Uninstall Kalama finished"
echo "--------------------------------------------------------------------------"
echo "--------------------------------------------------------------------------"
exit 0